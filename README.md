...
```bash
# Интерактивный (ставите оценки 0-3 вручную)
python scripts/benchmark.py --tag "v1: base model" --save

# Только вывод результатов без оценок
python scripts/benchmark.py --no-interactive
```

Результаты сохраняются в `logs/benchmark.jsonl`.  
Запускайте перед и после каждого изменения (модель, размер чанка, efSearch) — сравнивайте баллы.
